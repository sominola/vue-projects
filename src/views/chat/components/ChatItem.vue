...
<template>
  <router-link :to="'/chat/' + chat.id" v-if="user" class="bg-accents-1 flex justify-between 
     h-[75px] items-center p-3 hover:cursor-pointer hover:bg-accents-2"
    ...
    <AvatarRoot v-if="chat.type == ChatType.Group"
      ...
      <AvatarImage class="h-full w-full rounded-[inherit] object-cover" :src="chat.avatarUrl ?? avatarUrl"
                   alt="Avatar"/>
    </AvatarRoot>
    <AvatarRoot v-if="chat.type == ChatType.Personal"
                class="bg-blackA3 inline-flex h-[60px] w-[60px] select-none items-center justify-center overflow-hidden rounded-full align-middle">
      <AvatarFallback
